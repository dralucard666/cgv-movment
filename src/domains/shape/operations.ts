import { from, map, Observable, of } from "rxjs"
import { LinePrimitive, PointPrimitive } from "."
import { Operations, simpleExecution } from "../.."
import {
    makeRotationMatrix,
    makeScaleMatrix,
    makeTranslationMatrix,
    createPhongMaterialGenerator,
    FacePrimitive,
    Primitive,
    Axis,
    Split,
} from "."
import { Box3, Color, Shape, ShapeUtils, Vector2, Vector3 } from "three"
import * as THREE from "three"
import { defaultOperations } from ".."
import { ObjectPrimitive } from "./primitive"
import { GLTFLoader } from "three-stdlib/loaders/GLTFLoader"
import { DRACOLoader } from "three-stdlib/loaders/DRACOLoader"
import { computeGableRoof } from "./roof"
import { createGraph, expandGraph, getDirection, YAXIS, ZAXIS } from "./primitive-utils"

function computeGraphExpand(instance: Primitive, distance: number, ...lines: Array<LinePrimitive>): Array<Primitive> {
    return expandGraph(
        createGraph(
            lines.map((line) => [line.getStart(), line.getEnd()]),
            YAXIS,
            distance
        ),
        distance,
        0,
        YAXIS,
        instance.materialGenerator
    )
}

THREE.Cache.enabled = true

const gltfLoader = new GLTFLoader()
const dracoLoader = new DRACOLoader()
dracoLoader.setDecoderPath("/cgv/")
gltfLoader.setDRACOLoader(dracoLoader)

function computeLoad(instance: Primitive, url: string) {
    let obje: any
    gltfLoader.load(url, (gltf) => [(obje = new ObjectPrimitive(instance.matrix, gltf.scene))])
    return obje
}

function computePoint3(instance: Primitive, x: number, y: number, z: number): Array<Primitive> {
    const redMaterialGenerator = createPhongMaterialGenerator(new Color(0xff0000))
    return [new PointPrimitive(makeTranslationMatrix(x, y, z, instance.matrix.clone()), redMaterialGenerator)]
}

function computePoint2(instance: Primitive, x: number, z: number): Array<Primitive> {
    const redMaterialGenerator = createPhongMaterialGenerator(new Color(0xff0000))
    return [new PointPrimitive(makeTranslationMatrix(x, 0, z, instance.matrix.clone()), redMaterialGenerator)]
}

const helperVector = new Vector3()

function computeFace(instance: Primitive, ...points: ReadonlyArray<Primitive>): Array<Primitive> {
    if (points.length < 3) {
        return [instance]
    }
    const points2d = points.map((point) => {
        helperVector.setFromMatrixPosition(point.matrix)
        return new Vector2(helperVector.x, helperVector.z)
    })
    if (ShapeUtils.isClockWise(points2d)) {
        points2d.reverse()
    }
    const redMaterialGenerator = createPhongMaterialGenerator(new Color(0xff0000))

    return [new FacePrimitive(instance.matrix, new Shape(points2d), redMaterialGenerator)]
}

function computeLine(instance: Primitive, ...points: ReadonlyArray<Primitive>): Array<Primitive> {
    if (points.length < 2) {
        return [instance]
    }
    const [p1, p2] = points.map((point) => new Vector3().setFromMatrixPosition(point.matrix))
    const redMaterialGenerator = createPhongMaterialGenerator(new Color(0xff0000))
    return [LinePrimitive.fromPoints(instance.matrix.clone(), p1, p2, redMaterialGenerator)]
}

function computeSample(instance: Primitive, amount: number): Array<Primitive> {
    return instance.samplePoints(amount)
}

const size = new Vector3()
const box3Helper = new Box3()

function computeSize(instance: Primitive, axis: "x" | "y" | "z"): Array<any> {
    instance.getBoundingBox(box3Helper)
    box3Helper.getSize(size)
    return [size[axis as keyof Vector3]]
}

function computeScale(instance: Primitive, x: number, y: number, z: number): Array<Primitive> {
    return [instance.multiplyMatrix(makeScaleMatrix(x, y, z))]
}

function degreeToRadians(degree: number): number {
    return (Math.PI * degree) / 180
}

function computeRotate(instance: Primitive, x: number, y: number, z: number): Array<Primitive> {
    return [instance.multiplyMatrix(makeRotationMatrix(degreeToRadians(x), degreeToRadians(y), degreeToRadians(z)))]
}

function computeTranslate(instance: Primitive, x: number, y: number, z: number): Array<Primitive> {
    return [instance.multiplyMatrix(makeTranslationMatrix(x, y, z))]
}

function computeColorChange(instance: Primitive, color: Color): Array<Primitive> {
    return [instance.changeMaterialGenerator(createPhongMaterialGenerator(color))]
}

function computeExtrude(instance: Primitive, by: number): Array<Primitive> {
    return [instance.extrude(by)]
}

function computeComponents(type: "points" | "lines" | "faces", ...instances: Array<Primitive>): Array<Primitive> {
    const components = instances.reduce<Array<Primitive>>(
        (instances, instance) => [...instances, ...instance.components(type)],
        []
    )
    return components
}

function computeSplit(
    instance: Primitive,
    axis: Axis,
    repetitions: boolean | number,
    ...sizes: Array<number>
): Array<Primitive> {
    if (sizes.length === 0) {
        return [instance]
    }

    const splits = Split(instance, axis, (matrix, index, x, y, z) => {
        const repetitionIndex = Math.floor(index / sizes.length)
        const size = sizes[index % sizes.length]
        if ((repetitions === false && repetitionIndex == 0) || repetitions === true || repetitionIndex < repetitions) {
            const sizeX = axis === "x" ? Math.min(size, x) : x
            const sizeZ = axis === "z" ? Math.min(size, z) : z
            return FacePrimitive.fromLengthAndHeight(matrix, sizeX, sizeZ, ZAXIS, instance.materialGenerator)
        } else {
            return FacePrimitive.fromLengthAndHeight(matrix, x, z, ZAXIS, instance.materialGenerator)
        }
    })
    return splits
}

function computeDirection(instance: Primitive): Array<any> {
    return [getDirection(instance.matrix)]
}

export const operations: Operations<any> = {
    ...defaultOperations,
    translate: {
        execute: simpleExecution<any>(computeTranslate),
        includeThis: true,
        changesTime: false,
        defaultParameters: [
            () => ({ type: "raw", value: 0 }),
            () => ({ type: "raw", value: 0 }),
            () => ({ type: "raw", value: 0 }),
        ],
    },
    scale: {
        execute: simpleExecution<any>(computeScale),
        includeThis: true,
        changesTime: false,
        defaultParameters: [
            () => ({ type: "raw", value: 1 }),
            () => ({ type: "raw", value: 1 }),
            () => ({ type: "raw", value: 1 }),
        ],
    },
    rotate: {
        execute: simpleExecution<any>(computeRotate),
        includeThis: true,
        changesTime: false,
        defaultParameters: [
            () => ({ type: "raw", value: 0 }),
            () => ({ type: "raw", value: 0 }),
            () => ({ type: "raw", value: 0 }),
        ],
    },
    extrude: {
        execute: simpleExecution<any>(computeExtrude),
        includeThis: true,
        changesTime: false,
        defaultParameters: [() => ({ type: "raw", value: 1 })],
    },
    split: {
        execute: simpleExecution<any>(computeSplit),
        includeThis: true,
        changesTime: false,
        defaultParameters: [
            () => ({ type: "raw", value: "x" }),
            () => ({ type: "raw", value: true }),
            () => ({ type: "raw", value: 1 }),
        ],
    },
    toPoints: {
        execute: simpleExecution<any>(computeComponents.bind(null, "points")),
        includeThis: true,
        changesTime: false,
        defaultParameters: [],
    },
    toLines: {
        execute: simpleExecution<any>(computeComponents.bind(null, "lines")),
        includeThis: true,
        changesTime: false,
        defaultParameters: [],
    },
    toFaces: {
        execute: simpleExecution<any>(computeComponents.bind(null, "faces")),
        includeThis: true,
        changesTime: false,
        defaultParameters: [],
    },
    sample: {
        execute: simpleExecution<any>(computeSample),
        includeThis: true,
        changesTime: false,
        defaultParameters: [() => ({ type: "raw", value: 10 })],
    },
    color: {
        execute: simpleExecution<any>(computeColorChange),
        includeThis: true,
        changesTime: false,
        defaultParameters: [() => ({ type: "raw", value: "#ff0000" })],
    },
    size: {
        execute: simpleExecution<any>(computeSize),
        includeThis: true,
        changesTime: false,
        defaultParameters: [() => ({ type: "raw", value: "x" })],
    },
    /*mapbox: {
        execute: simpleExecution<any, unknown>(computeMapbox),
        includeThis: true,
        defaultParameters: [],
    },*/
    expandGraph: {
        execute: simpleExecution<any>(computeGraphExpand),
        includeThis: true,
        changesTime: false,
        defaultParameters: [() => ({ type: "raw", value: 3 })],
    },
    load: {
        execute: simpleExecution<any>(computeLoad),
        includeThis: true,
        changesTime: false,
        defaultParameters: [],
    },
    point3: {
        execute: simpleExecution<any>(computePoint3),
        includeThis: true,
        changesTime: false,
        defaultParameters: [
            () => ({ type: "raw", value: 0 }),
            () => ({ type: "raw", value: 0 }),
            () => ({ type: "raw", value: 0 }),
        ],
    },
    point2: {
        execute: simpleExecution<any>(computePoint2),
        includeThis: true,
        changesTime: false,
        defaultParameters: [() => ({ type: "raw", value: 0 }), () => ({ type: "raw", value: 0 })],
    },
    line: {
        execute: simpleExecution<any>(computeLine),
        includeThis: true,
        changesTime: false,
        defaultParameters: [
            () => ({
                type: "operation",
                identifier: "point3",
                changesTime: false,
                children: [
                    { type: "raw", value: 0 },
                    { type: "raw", value: 0 },
                    { type: "raw", value: 0 },
                ],
            }),
            () => ({
                type: "operation",
                identifier: "point3",
                changesTime: false,
                children: [
                    { type: "raw", value: 1 },
                    { type: "raw", value: 0 },
                    { type: "raw", value: 0 },
                ],
            }),
        ],
    },
    face: {
        execute: simpleExecution<any>(computeFace),
        includeThis: true,
        changesTime: false,
        defaultParameters: [],
    },
    gableRoof: {
        execute: simpleExecution<any>(computeGableRoof),
        includeThis: true,
        changesTime: false,
        defaultParameters: [
            () => ({
                type: "raw",
                value: 0,
            }),
        ],
    },
    direction: {
        execute: simpleExecution<any>(computeDirection),
        includeThis: true,
        changesTime: false,
        defaultParameters: [],
    },
}
